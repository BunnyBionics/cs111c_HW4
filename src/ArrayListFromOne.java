...
public class ArrayListFromOne<T extends Comparable<? super T>> implements ListFromOne<T>, Comparable<ArrayListFromOne<T>> {
	...
	public int removeAll(T element) {
		...
		T[] newArray = (T[]) new Comparable[listArray.length];
		int newSize = 0;
		int removals = 0;
		...
		for (int i = 1; i < this.size() + 1; i++) {
			if (listArray[i].equals(element)) {
				...
				newArray[1 + newSize++] = listArray[i];
			}
		}
	...
	public boolean equals(Object other) {
		...
		ArrayListFromOne<T> that = (ArrayListFromOne<T>) other;
		...
		for (int i = 1; i < size + 1; i++) {
			if (!this.listArray[i].equals(that.listArray[i])) {
				return false;
			}
		}
	...
	public int compareTo(ArrayListFromOne<T> otherList) {
		...
		for (int i = 1; i < this.size + 1; i++) {
			if (!this.listArray[i].equals(otherList.listArray[i])) {
				return this.listArray[i].compareTo(otherList.listArray[i]);
			}
		}
